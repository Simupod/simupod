...
:func:`~simupod.plugins.mode_overlap._cell_widths`), so graded / non-uniform
...
def _solid_angle_weights(theta: np.ndarray, phi: np.ndarray) -> np.ndarray:
    ...
    (reusing :func:`~simupod.plugins.mode_overlap._cell_widths`)."""
...
def far_field(
    ...
    *,
    ...
    ----------
    data:
        A :class:`~simupod.data.SimulationData` (or any mapping) such that
