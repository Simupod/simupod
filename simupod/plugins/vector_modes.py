...
:mod:`simupod.plugins.modes`. Where that solver carries a single dominant
...
* ``VectorModeSolver(eps, dl_x_um, dl_y_um, wavelength_um)`` — same raw-eps
  validation as :class:`simupod.plugins.modes.ModeSolver`.
...
try:  # scipy is a hard dependency of the full-vector solver (sparse eigensolve)
    ...
    raise ImportError(
        "simupod.plugins.vector_modes requires scipy (sparse matrices + "
...
def _odd(n: int) -> int:
    ...
    the cross-section origin — mirrors :mod:`simupod.plugins.modes`)."""
...
class VectorModeSolver:
    ...
    #: Speed of light in vacuum (m/s), matching :mod:`simupod.plugins.modes`.
    ...
    def from_rectangular_core(
        ...
        *,
        ...
        :meth:`simupod.plugins.modes.ModeSolver.from_rectangular_core`, so the
