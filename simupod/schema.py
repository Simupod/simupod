...
``schemas/simulation_v1.json`` is generated:
    ...
    python -m simupod.schema emit  [path]   # (re)write the schema file
    python -m simupod.schema check [path]   # exit 1 if the file is stale
...
def check(path: Path) -> int:
    ...
    if not path.is_file():
        print(f"schema check FAILED: {path} does not exist; "
              "run 'python -m simupod.schema emit'", file=sys.stderr)
        return 1
